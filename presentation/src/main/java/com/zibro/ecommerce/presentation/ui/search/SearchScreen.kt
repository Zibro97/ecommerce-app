        ...
        sheetContent = {
            when(currentFilterType) {
                SearchFilter.Type.CATEGORY -> {
                    val category = searchFilter.first { it is SearchFilter.CategoryFilter } as SearchFilter.CategoryFilter
                    SearchFilterCategoryContent(filter = category) {
                        scope.launch {
        ...
        LazyColumn(
            ...
            items(filter.categories.size) { index ->
                ...
                Button(
                    ...
                    colors = ButtonDefaults.buttonColors(backgroundColor = if (filter.selectedCategory == category) Color.LightGray else Color.Unspecified)
                ) {
                    Text(
...
fun SearchFilterPriceContent(
    ...
    Column(
        ...
        Text(text = "최저가 : ${sliderValues.start.toInt()} ~ 최고가 : ${sliderValues.endInclusive.toInt()}")
    }
}
